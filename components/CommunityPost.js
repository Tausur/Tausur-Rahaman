    ...
    <div>
      ...
      <div className='py-2 md:px-32 px-5 md:w-[1200px] m-auto'>
        <div className='bg-gray-800 rounded-xl md:pt-4 shadow-2xl'>
          ...
          <div className='md:text-xl text-md px-5 py-3'>
            {community.post}
          </div>
          {
          ...
          <div className='flex items-center md:pl-5 md:py-2 md:pb-5 py-2'>
            <div className='flex items-center'>
              {like == false ? <BiLike className='text-white md:text-2xl text-lg md:ml-5 cursor-pointer ml-8 mx-1' onClick={handleLike} /> : <BiLike className='text-blue-500 md:text-2xl text-lg md:ml-5 cursor-pointer ml-8 mx-1' onClick={handleLike} />
              }
            </div>
            <BiCommentDetail className='mx-8 cursor-pointer text-xl' onClick={handleComment} />
          </div>
          {myCom == true ? <div>
            <div>
              <form className='md:px-8 md:pt-2 md:pb-5' onSubmit={handler}>
                <label className='md:text-xl text-lg font-semibold block pb-3 md:px-0 px-5 md:py-0 py-5'>Post a comment</label>
                <div>
                  <input type="text" className='block focus:outline-none border-b-2 bg-gray-800 md:text-lg text-md w-1/2 md:my-4 my-2 md:mx-0 mx-5' placeholder='Your name' value={name} onChange={(e) => setName(e.target.value)} />
                  <div className='flex items-center'>
                    <input type="text" className='focus:outline-none border-b-2 bg-gray-800 md:text-lg text-md w-4/5 md:w-1/2 md:mx-0 ml-5 md:ml-0 md:mr-0 mr-2 md:my-0 my-2' placeholder='Your message' value={msg} onChange={(e) => setMsg(e.target.value)} />
                    <button type='submit'>
                ...
                {myabc.map((comment) => {
                  return <div className='md:py-0 py-1'>
                    <div className='flex items-center'>
                      ...
                      <div className='md:text-xl text-md font-semibold pl-2 text-white'>
                        {comment.name}
                      </div>
                    </div>
                    <div className='md:px-20 px-12 md:text-lg text-md'>
                      {comment.msg}
                    </div>
                  </div>
                })}
